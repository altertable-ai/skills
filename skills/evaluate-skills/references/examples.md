...
- Data question? → explore-data
- Need query? → query-lakehouse
- Chart output? → analyze-insights
```
...
|--------|--------|-------------|
| "chart" in query | High | analyze-insights |
| "SQL" in query | High | query-lakehouse |
| "what data" | Medium | explore-data |
```
